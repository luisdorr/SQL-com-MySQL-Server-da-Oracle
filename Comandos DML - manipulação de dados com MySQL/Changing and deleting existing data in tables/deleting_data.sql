...
SELECT * FROM products WHERE SUBSTRING(DESCRIPTOR, 1, 15) = 'Sabor dos Alpes';

DELETE FROM products WHERE  CODE = '1001011';

SELECT CODE
FROM products
WHERE CODE NOT IN (SELECT CODIGO_DO_PRODUTO FROM sucos_vendas.tabela_de_produtos);
...
SELECT A.NUMBER FROM notes A
INNER JOIN customer B ON A.CPF = B.CPF
WHERE B.AGE <= 18;
